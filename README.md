...
The local Whisper fallback uses `backend/voice/faster_whisper_transcribe.py`. By default it uses model `base`, language `en`, and device `auto`, which means CUDA is used when CTranslate2 can see a GPU and CPU is used otherwise. You can override it before starting the server:
...
`faster-whisper` only converts audio into text. DOC's actual assistant behavior comes from `backend/voice/assistantLibrary.js` and `backend/voice/voice_parser.py`: greetings, identity questions, slot prompts, medical intake extraction, and report commands are handled there.
...
11. Rejected uploads are logged in `database/training/audit-log.json`.
...
- Tokenization: `backend/training/vectorStore.js`
- Vectorization: normalized term-frequency vectors
- Similarity: cosine similarity
- Forward/backward propagation classifier: `backend/training/neuralTrainer.js`
- Training ingestion: `backend/training/fileProcessor.js`
- External dataset import: `backend/scripts/importMedicalDataset.js`
- Report generation: `ai/medicalEngine.js`
- Chat state: `ai/chatAssistant.js`
- Voice assistant response library: `backend/voice/assistantLibrary.js`
- Python voice parsing: `backend/voice/voice_parser.py`
- Optional faster-whisper transcription: `backend/voice/faster_whisper_transcribe.py`
- Trusted online research: `backend/training/remoteKnowledge.js`
...
- `database/training/manual-knowledge.json`
- `database/training/user-learning.json`
- `database/training/audit-log.json`
...
```text
database/seed-medical-knowledge.json
```
...
Voice understanding uses `backend/voice/assistantLibrary.js` and `backend/voice/voice_parser.py`, so answers do not need one exact phrase format. For example, when DOC asks for a name, `Rahul`, `my name is Rahul`, and `patient name is Rahul Kumar` are all accepted. When DOC asks age, `24`, `I am 24`, or `24 years old` are accepted. Conversational turns such as `hello`, `what is your name`, `how are you`, `what is your age`, `what can you do`, `repeat`, and `thank you` are answered as assistant interactions instead of being stored as symptoms or patient names.
...
- visible text in `frontend/*.html`
- `package.json` name/description
- README title and wording
- generated report title in `backend/reports/reportRenderer.js`
...
User browser
  |
  |-- frontend/index.html (landing screen and main menu)
  |-- frontend/app.html (manual intake form)
  |-- frontend/chat.html (voice assistant screen)
  |-- frontend/result.html (report viewer and feedback form)
  |
  v
backend/server.js (local HTTP server and API router)
  |
  ...
  |
  v
ai/medicalEngine.js (urgency, disease matching, medicine filtering, report JSON)
  |
  |-- backend/training/vectorStore.js (raw text -> tokens -> normalized vectors -> cosine search)
  |-- backend/training/neuralTrainer.js (forward propagation + backward propagation classifier)
  |-- backend/training/fileProcessor.js (file extraction, validation, duplicate handling, training storage)
  |-- backend/training/remoteKnowledge.js (trusted MedlinePlus/NLM fallback when local knowledge is weak)
  |
  v
backend/reports/reportRenderer.js (turns report JSON into printable/downloadable HTML)
```

```text
DOC project tree
|-- package.json (run scripts and project metadata)
|-- requirements.txt (optional faster-whisper Python dependency)
|-- frontend/
|   |-- js/
