...
const { extractTextFromFile } = require("../training/fileProcessor");
const { searchDocuments, normalizeText } = require("../training/vectorStore");
const { buildNeuralConditionModel, predictCondition } = require("../training/neuralTrainer");
...
CONDITION_LIBRARY.push(
  ...
  },
  {
    ...
    triggers: ["anemia", "anaemia", "tired", "fatigue", "pale", "dizzy", "short breath on exertion", "heavy periods", "low hemoglobin"],
    explanation:
...
function detectUrgency(text) {
  ...
  if (/severe|worsening|high fever|unable|cannot move|deform|pregnan|infant|elderly/i.test(text)) {
    return {
      level: "Soon",
      reasons: ["Symptoms include severity or risk words. Same-day medical advice is safer."]
    };
  }
  return {
    level: "Self-care with monitoring",
